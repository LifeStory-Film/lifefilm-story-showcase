...
// Venue data - add new venues here
const venueData: Record<string, { name: string; instagram?: string; email?: string }> = {
  'vistonianvenue': { name: 'The Vistonian', instagram: '@thevistonian' },
  'stoneoakmanor': { name: 'Stone Oak Manor', instagram: '@stoneoakmanor' },

  // TEMPLATE fallback (do not delete)
  template: { name: '[VENUE NAME]', instagram: '' },
}

export default function VenueReferralPage() {
  ...
  // â Compute slug + venuer AFTER hooks (still no conditional hooks)
  const venueSlug = useMemo(() => {
    const raw = (params?.venue ?? '') as string | string[]
    return Array.isArray(raw) ? raw[0] : raw
  }, [params])

  const venue = useMemo(() => {
    return venueData[venueSlug] || venueData.template
  }, [venueSlug])
      ...
      <section className="pt-36 pb-20 px-6">
        ...
        >
          ...
          <p className="text-lg text-[#525252] font-light leading-relaxed">
            Photography and cinematic videography for couples working alongside {venue.name}.
          </p>

{(venue.instagram || venue.email) && (
  <p className="mt-4 text-sm text-[#737373] font-light">
    Venue:{' '}
    {venue.instagram && (
      <span className="text-[#1a1a1a]">{venue.instagram}</span>
    )}
    {venue.email && (
      <>
        {venue.instagram && ' Â· '}
      ...
      <section className="py-16 px-6 bg-white">
        <div className="max-w-xl mx-auto text-center">
          <p className="text-lg text-[#404040] font-light leading-relaxed">
            Our work aligns closely with {venue.name}
      ...
      <section className="py-24 px-6 bg-gradient-to-b from-white via-[#f8f8f7] to-[#f5f5f4]">
        <div className="max-w-3xl mx-auto">
          ...
          <h2 className="text-2xl md:text-[1.75rem] font-light text-center mb-12 font-serif text-[#1a1a1a]">
            A Thoughtful Thank You for Working with {venue.name}
          </h2>
      ...
      <section className="py-12 px-6 bg-[#f5f5f4]">
        <div className="max-w-xl mx-auto text-center">
          <p className="text-sm text-[#737373]">
            You{"'"}ve arrived through {venue.name}
      ...
      <section id="availability" className="py-28 px-6 bg-gradient-to-b from-[#fafaf9] to-[#f0f0ef]">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="text-[#525252] font-light mb-8 max-w-md mx-auto">
            You{"'"}re here because {venue.name} believes we{"'"}d be a great fit. We{"'"}d love to learn more about your day.
          </p>

          <a
            href={`mailto:rick@lifestory.film?subject=Inquiry%20via%20${encodeURIComponent(venue.name)}&body=Hi%20Rich%2C%0A%0AI%20was%20referred%20by%20${encodeURIComponent(
              venue.name
